        ...
        do{
            ...
            for (int j = 1; j <= A.nrows(); j++)
                for (int i = 1; i <= rank_of_A; i++)
                    Column_reduced_version_of_A(j,i) = B[j-1][i-1];
            
            //cout<<"A'"<<endl<<Column_reduced_version_of_A<<endl;
            ...
            if (Rank(Column_reduced_version_of_A) == rank_of_A)
            {
                ...
                for (int t = 1; t <= A.ncols(); ++t)
                {
                    
                    if (did_i_pick_this_col[t-1] == true)
                    {
                        
                        temp.push_back(soln(ind));
                        ind = ind+1;
                    }
                    else
                    {
...
void trim_the_solution_set(Matrix A, ColumnVector y)
{
    ...
    //cout<<"The solution matrix is: "<<endl<<solution<<endl;
    ...
    
    {
        ...
        do
        {
            ...
            for (int i = 0; i < solution.nrows(); ++i)
            {
                ...
                {
                    if (did_i_pick_this[k] == true)
                    {
                        double num = solution(i+1,k+1);
                        v1.push_back(num);
                    }
                    else
                        continue;
            ...
            for (int j = 1; j <= solution.nrows(); j++)
                for (int k = 1; k <= rank_of_solution_set; k++)
                    Column_reduced_version(j,k) = B[j-1][k-1];
            
            //cout<<"Column_reduced_version: "<<endl<<Column_reduced_version<<endl;
...
void process_data(char* argv[])
{
    ...
    if (input_file.is_open()) // If the input file exists in the local directory
    {
        ...
        input_file >> no_of_cols; // read the number of cols of A matrix
